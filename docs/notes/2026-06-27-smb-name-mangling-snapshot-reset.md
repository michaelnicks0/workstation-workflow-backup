...
| Sensitivity | local-private; no credentials or payload contents included |
...
- The live NAS share `ws1-wf` now has `mangled names = no`, and a non-mutating Robocopy dry-run reports `0 copied / 0 extras`.
- All stale `v1/ws1/wf` snapshots were explicitly approved as cruft and purged; a successful backup run and fresh baseline snapshot were created.
...
3. ✅ Disabled SMB name mangling on the `ws1-wf` share and made the setting durable in `scripts/nas-provision.sh`.
4. ✅ Purged approved stale snapshots under `v1/ws1/wf` only.
...
- `v1/ws1/wf` had about `515.59 GiB` snapshot-held data against a `512 GiB` snapshot budget.
...
- TrueNAS/Samba effective config originally exposed name mangling behavior; setting `mangled names = no` for the `ws1-wf` share removed the loop.
...
| Fix | Disable name mangling for backup share | `mangled names = no` under `[ws1-wf]` |

### Live SMB change

The live TrueNAS SMB share was updated via middleware for share ID `3` / `ws1-wf`:
...
Only snapshots under `v1/ws1/wf` were destroyed; the mutable `current/...` mirror was not deleted.
...
```text
v1/ws1/wf@post-snapshot-reset-20260628-012002
USED 0B
REFER 120G
```
...
| Live TrueNAS/Samba | `midclt`, `testparm` | Verified `mangled names = no` is effective for `[ws1-wf]`. |
